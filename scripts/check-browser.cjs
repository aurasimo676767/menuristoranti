...
(async()=>{
  ...
  try {
    ...
    assert.equal(await page.locator('.dish-card').nth(12).evaluate(el=>getComputedStyle(el).opacity),'0');
    await page.locator('.dish-card').nth(12).scrollIntoViewIfNeeded();
    await page.waitForFunction(()=>document.querySelectorAll('.dish-card')[12].classList.contains('dish-arrived'));
    assert.equal(await page.locator('.dish-card').nth(12).evaluate(el=>getComputedStyle(el).animationName),'dish-rise');
    await page.locator('.dish-card').nth(12).evaluate(el=>Promise.all(el.getAnimations().map(animation=>animation.finished)));
    assert.equal(await page.locator('.dish-card').nth(12).evaluate(el=>getComputedStyle(el).opacity),'1');
    await page.locator('.menu-intro').scrollIntoViewIfNeeded();
    await page.waitForFunction(()=>document.querySelector('.menu-intro').classList.contains('section-arrived'));
    await page.locator('.dish-card').first().click();
    assert.equal(await page.locator('.supplement-spotlight').first().evaluate(el=>getComputedStyle(el).animationName),'section-arrive');
    for(const width of [320,390,1366]){
      await page.setViewportSize({width,height:900});
      assert.ok(await page.locator('#dish-dialog').evaluate(el=>el.scrollWidth<=el.clientWidth),`Overflow dettagli animati a ${width}px`);
    }
    await page.keyboard.press('Escape');
    await page.setViewportSize({width:1366,height:950});
    await page.locator('.dish-card').first().hover();
    assert.equal(await page.locator('.dish-card').first().evaluate(el=>getComputedStyle(el,'::before').opacity==='0'),false);
    await page.screenshot({path:path.join(os.tmpdir(),'andrea-menu-aura.png')});
    await page.locator('.hero-visual').scrollIntoViewIfNeeded();
    await page.waitForFunction(()=>!document.querySelector('.hero-visual').classList.contains('motion-paused'));
    ...
    assert.equal(await page.locator('.menu-intro').evaluate(el=>getComputedStyle(el).animationName),'none');
    await page.locator('.dish-card').first().click();
    assert.equal(await page.locator('.supplement-spotlight').first().evaluate(el=>getComputedStyle(el).animationName),'none');
    await page.keyboard.press('Escape');
