...
(async () => {
  ...
  try {
    ...
    await page.locator('#offer-popup').check();
    ...
    assert.equal(await customer.locator('#offers').isVisible(), false);
    ...
    await customer.reload(); await customer.locator('#offers-grid .offer-card').waitFor();
    ...
    await page.locator('#publish').click();
    await page.waitForFunction(() => document.querySelector('#status').textContent.startsWith('Offerte pubblicate'));
    await customer.reload(); await customer.locator('.dish-card').first().waitFor();
    assert.equal(await customer.locator('#offers').isVisible(), false);
